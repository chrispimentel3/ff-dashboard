...
def trade_options(player: str, yahoo_rosters: pd.DataFrame | None = None,
                  band: float = 0.25, max_ideas: int = 14) -> dict:
    """One named player, and the trades that could actually happen around him.

    The two directions are the same machinery from opposite ends. If he is yours, the
    question is what comes back; if he is someone else's, it is what it would take. Either
    way an offer only appears when both sides can say yes: the values are within `band` of
    each other, and giving him up doesn't open a hole where you were already thin.

    Returns {player, pos, owner, side, value, ideas, note}. `side` is "mine", "theirs" or
    "free"; `ideas` is empty when nothing clears the band, and `note` says why.
    """
    ros = _valued_rosters(yahoo_rosters)
    if ros.empty:
        return {"player": player, "side": "none", "ideas": pd.DataFrame(),
                "note": "No rosters with trade values — the Yahoo pull may not have run."}

    key = _norm(player)
    hit = ros[ros["norm"] == key]
    if hit.empty:
        return {"player": player, "side": "free", "ideas": pd.DataFrame(),
                "note": f"{player} isn't on a Mega Bowl roster with a trade value — "
                        "if he's a free agent, claim him rather than trade for him."}
    him = hit.iloc[0]
    mine_team = str(him["team"]) == MY_TEAM

    strength = _positional_strength(ros)
    lg_avg = strength.groupby("pos")["starter_value"].mean()
    def needs(team):
        t = strength[strength["team"] == team].set_index("pos")["starter_value"]
        gap = (t - lg_avg).sort_values()
        return list(gap[gap < 0].index), list(gap[gap > 0].index)
    my_need, my_surplus = needs(MY_TEAM)

    def fair(a, b):
        top = max(a, b)
        return 0.0 if top == 0 else round(1 - abs(a - b) / top, 2)

    hv = float(him["value"])
    mine = ros[ros["team"] == MY_TEAM]
    rows = []

    if mine_team:
        # Shopping him: what comes back, and who has a reason to want him.
        for team in sorted(set(ros["team"]) - {MY_TEAM}):
            their_need, _ = needs(team)
            wants_him = him["pos"] in their_need
            for _, get in ros[ros["team"] == team].iterrows():
                gv = float(get["value"])
                if get["norm"] == key or abs(gv - hv) > band * max(gv, hv):
                    continue
                fills = get["pos"] in my_need
                rows.append(dict(
                    partner=team, get=get["player"], get_pos=get["pos"], get_val=int(gv),
                    fairness=fair(hv, gv),
                    addresses=get["pos"] if fills else "—",
                    why_them=(f"thin at {him['pos']}" if wants_him else "value only"),
                    _score=(gv - hv) + (600 if fills else 0) + (400 if wants_him else 0)
                            + 0.15 * (float(get.get("trend_30d") or 0) - float(him.get("trend_30d") or 0)),
                ))
        cols = ["partner", "get", "get_pos", "get_val", "addresses", "why_them", "fairness"]
        note = (f"Giving up your only startable {him['pos']} — make sure the return covers it."
                if him["pos"] in my_need else "")
    else:
        # Chasing him: what of yours gets it done without opening a hole.
        their_need, _ = needs(str(him["team"]))
        wants = set(their_need)
        for _, give in mine.iterrows():
            gv = float(give["value"])
            if give["norm"] == key or abs(gv - hv) > band * max(gv, hv):
                continue
            costly = give["pos"] in my_need and give["pos"] != him["pos"]
            rows.append(dict(
                partner=str(him["team"]), give=give["player"], give_pos=give["pos"],
                give_val=int(gv), fairness=fair(hv, gv),
                why_them=("fills their " + give["pos"] if give["pos"] in wants else "value only"),
                costs_you=("opens a hole at " + give["pos"]) if costly else "—",
                _score=(hv - gv) + (500 if give["pos"] in wants else 0) - (700 if costly else 0)
                        + (300 if give["pos"] in my_surplus else 0),
            ))
        cols = ["partner", "give", "give_pos", "give_val", "why_them", "costs_you", "fairness"]
        note = ("They're thin at " + ", ".join(their_need[:2]) + " — lead with that."
                if their_need else "")

    ideas = pd.DataFrame(rows)
    if ideas.empty:
        closest = (mine if not mine_team else ros[ros["team"] != MY_TEAM])
        gap = ""
        if not closest.empty:
            near = closest.iloc[(closest["value"] - hv).abs().argsort()[:1]]
            gap = (f" The closest single piece is {near.iloc[0]['player']} at "
                   f"{int(near.iloc[0]['value'])} against his {int(hv)}.")
        return {"player": him["player"], "pos": him["pos"], "owner": str(him["team"]),
                "side": "mine" if mine_team else "theirs", "value": int(hv),
                "ideas": ideas,
                "note": f"Nothing lands within {band:.0%} of his value one-for-one, so this "
                        f"one needs a two-for-one.{gap}"}

    ideas = (ideas.sort_values("_score", ascending=False)
             .drop_duplicates(cols[1]).head(max_ideas).reset_index(drop=True))
    return {"player": him["player"], "pos": him["pos"], "owner": str(him["team"]),
            "side": "mine" if mine_team else "theirs", "value": int(hv),
            "ideas": ideas[cols], "note": note}
